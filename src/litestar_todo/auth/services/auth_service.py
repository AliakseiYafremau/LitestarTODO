...
from litestar_todo.auth.dto import UserScheme
...
class AuthService:
    ...
    async def authenticate(self, username: str, password: str) -> bool:
        ...
        if not user:
            return False  # Должно вызывать исключение
        return await self.verify_password(password, user.password)  # Тут тоже
    ...
    async def generate_token(self, user_id: str, secret: str, algorithm: str) -> str:
